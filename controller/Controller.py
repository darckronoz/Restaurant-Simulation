...
class Controller:
    ...
    def create_plates(self):
        ...
        plateTwo = Plate(2,"Bistec a Caballo", 1, 10, 2, 1, 2)
        plateThree = Plate(3,"Frijoles con Cayo", 5, 5, 2, 2, 1)
    ...
    def assign_order_to_customer(self,capacidad):
        ...
        for i in range(ran.randint(1,capacidad)):
            list.append(self.plateList[ran.randint(1, len(self.plateList))])
        return list
    ...
    def revision(self):
        ...
        for i in mesasList:
            i.start()
            if len(i.customersQueue) > 0:
                lm.append(i.customersQueue)
                print("hbbhbhbhhb", lm)
